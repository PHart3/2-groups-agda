...
module KLoop-ptr-comp-defs  {i j k} {X : Type i} {Y : Type j} {Z : Type k}
  ...
  σ₄ =
    ...
    ! (K₂-map-β-pts (cohsgrphom (ap g) {{Loop2Grp-map-str (g , idp)}} ∘2Mσ cohsgrphom (ap f) {{Loop2Grp-map-str (f , idp)}}) x) ◃∎
  ...
  ν₄ = 
    (λ v →
      ...
      ! (K₂-map-β-pts (cohsgrphom (ap g) {{Loop2Grp-map-str (g , idp)}} ∘2Mσ cohsgrphom (ap f) {{Loop2Grp-map-str (f , idp)}}) v))
