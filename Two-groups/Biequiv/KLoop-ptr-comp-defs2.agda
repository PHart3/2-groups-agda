...
module KLoop-ptr-comp-defs2  {i j k} {X : Type i} {Y : Type j} {Z : Type k}
  ...
  Λ₁ =     
    ...
    idp ◃∙
    ap (λ q → q)
      (! (ap (λ q → q) (ap (λ q → q) (∘-ap (fst (K₂-rec-hom z₀ idf2G)) (λ z → fst (K₂-map⊙ (Loop2Grp-map-str (g , idp)) ⊙∘ K₂-map⊙ (Loop2Grp-map-str (f , idp))) z) (loop (x₀ == x₀) x))))) ◃∙
    idp ◃∙
    idp ◃∙
    ...
    ! (ap (ap (fst (K₂-rec-hom z₀ idf2G))) (K₂-map-β-pts (cohmaghom (ap g) {{Loop2Grp-map-str (g , idp)}} ∘2Mσ cohmaghom (ap f) {{Loop2Grp-map-str (f , idp)}}) x)) ◃∙
    ! (ap (ap (fst (K₂-rec-hom z₀ idf2G))) (! (K₂-map-β-pts (cohmaghom (ap g) {{Loop2Grp-map-str (g , idp)}} ∘2Mσ cohmaghom (ap f) {{Loop2Grp-map-str (f , idp)}}) x))) ◃∙
