...
module KFCRU0 {i j} {G₁ : Type i} {G₂ : Type j} {{η₁ : CohGrp G₁}} {{η₂ : CohGrp G₂}}
  ...
  private
    ν₁ =
      λ v →
        K₂-map-β-pts σ v ∙
        (! (K₂-map-β-pts (cohsgrphom f {{σ}} ∘2Mσ cohsgrphom (λ z → z) {{idf2G}}) v) :> (loop G₂ (f v) == ap (K₂-map (cohsgrphom f {{σ}} ∘2Mσ cohsgrphom (λ z → z) {{idf2G}})) (loop G₁ v)))
    ν₂-suff =
      λ v →
        ! (K₂-map-β-pts σ v) ∙
    ...
    ν₂ =
      λ v →
        K₂-map-β-pts (cohsgrphom f {{σ}} ∘2Mσ cohsgrphom (λ z → z) {{idf2G}}) v ∙
  ...
  abstract
    ...
    K₂-β-1 :
      hmtpy-nat-∙' (fst (apK₂ (unit-wksgrphom-r (sgrphom-forg (cohsgrphom f {{σ}}))))) (loop G₁ x) ◃∎
        =ₛ
      K₂-map-β-pts σ x ◃∙
      ! (K₂-map-β-pts (cohsgrphom f {{σ}} ∘2Mσ cohsgrphom (λ z → z) {{idf2G}}) x) ◃∎
    K₂-β-1 = =ₛ-in $
      K₂-∼-ind-β (K₂-map σ) (K₂-map (cohsgrphom f {{σ}} ∘2Mσ cohsgrphom (λ z → z) {{idf2G}}))
        (idp :> (base G₂ == base G₂))
        ν₁
        _ x
