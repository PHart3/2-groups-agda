...
module _ {i j} {G₁ : Type i} {G₂ : Type j} {{η₁ : CohGrp G₁}} {{η₂ : CohGrp G₂}} where
  ...
  CohGrpHom-Σ =
   equiv
     (λ (map , map-comp , map-al) → cohgrphom map {{cohsgrphomstr map-comp map-al}})
     (λ (cohgrphom map {{cohsgrphomstr map-comp map-al}}) → map , (map-comp , map-al))
     (λ (cohgrphom map {{cohsgrphomstr map-comp map-al}}) → idp)
...
module _ {i j} {G₁ : Type i} {G₂ : Type j} {{η₁ : CohGrp G₁}} {{η₂ : CohGrp G₂}} where
  ...
  natiso2G-to-==-! : {f : CohGrpHom {{η₁}} {{η₂}}}
    (iso₁ iso₂ : CohGrpNatIso f f) → ∼WkSGrpNatIso iso₁ (!ʷ iso₂)
