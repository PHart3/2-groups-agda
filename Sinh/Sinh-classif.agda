  ...
  NGrp-Sinh-≃ =
    ...
    [ (S n) , i , i ]-Groups-v2
      ≃⟨ (Σ-contr-red-cod {{λ {(BG , F , p)} → Π-level (λ u → ×-level
          ...
          (orthog-conn-trunc (λ x → Ω^'S-abgroup n (⊙[ fst (F u) , x ]) {{snd (snd (F u))}})
            (connected-≤T (≤T-ap-S 0≤T) {{fst (snd (F u))}})
            AbGrp-isGrpd))}})⁻¹ ⟩
    ...
    [ (X , cX , tX) ∈ [ S n , i ]-Groups ] ×
      ...
      ≃⟨ Σ-emap-r (λ (X , cX , tX) → Σ-emap-r (λ H → Σ-emap-r (λ F →
        ×-emap (Π-emap-r λ u → ×-emap (ide (Trunc -1 (F u))) (Π-emap-r (λ x → EM-Ω-==-ext n (H u) ⊙[ F u , x ])))
        (ide (F (pt X)))))) ⟩
    [ (X , cX , tX) ∈ [ S n , i ]-Groups ] ×
      [ H ∈ (de⊙ X → AbGroup i) ] ×
        [ F ∈ (de⊙ X → Type i) ] ×
          ...
          F (pt X)
      ≃⟨ Σ-emap-r (λ (X , cX , tX) → Σ-emap-r (λ H → Σ-emap-r (λ F →
           Σ-emap-r (λ _ → tors-ptd-is-trivial (⊙EM (H (pt X)) (S (S n))) {{PtdTorsors-contr {{EM-conn (H (pt X))}}}})))) ⟩
    [ (X , cX , tX) ∈ [ S n , i ]-Groups ] ×
      [ H ∈ (de⊙ X → AbGroup i) ] ×
        [ F ∈ (de⊙ X → Type i) ] ×
          [ d ∈ Π (de⊙ X) (λ u → Trunc -1 (F u) ×
            Π (F u) (λ x → ⊙EM (H u) (S (S n)) ⊙≃ ⊙[ F u , x ])) ] ×
          ((F (pt X) , d (pt X)) == pt (⊙Torsors (⊙EM (H (pt X)) (S (S n))) {{PtdTorsors-contr {{EM-conn (H (pt X))}}}})) 
    ...
    [ (X , cX , tX) ∈ [ S n , i ]-Groups ] ×
      ...
      ≃⟨ Σ-emap-r (λ (X , cX , tX) → Σ-emap-r (λ H → Π⊙-==-ext ((λ u → (EM-Torsors-≃ (H u)) ⁻¹) , ptdness X H))) ⟩
    [ (X , cX , tX) ∈ [ S n , i ]-Groups ] ×
      [ H ∈ (de⊙ X → AbGroup i) ] ×
      ...
      where
        ...
        rearrange = equiv
          ...
          (λ (BG@(X , cX , tX) , F , H , m , p) → (BG , ((λ u → (F u) , (fst (snd (m u)) , fst (snd (snd (m u))))) , p)) ,
        ...
        sub-rearrange : (X : Ptd i) (cX : is-connected 0 (de⊙ X))
          ...
          Σ (Π (de⊙ X) (λ z → Trunc -1 (F z))) (λ _ →
            Π (de⊙ X) (λ u → is-connected ⟨ S n ⟩ (F u) ×
              Σ (has-level ⟨ S (S n) ⟩ (F u)) (λ tF → Π (F u) (λ x → Ω^'S-abgroup n ⊙[ F u , x ] {{tF}} == H u)))) ×
          F (pt X)
            ≃
          Σ (Π (de⊙ X) (λ z → Trunc -1 (F z))) (λ _ →
        ...
        sub-rearrange X cX {F} {H} =
          equiv
            ...
            (λ ((t , m) , p) →
              (t ,
              (λ u →
                (prop-over-connected {{cX}} (λ x → is-connected ⟨ S n ⟩ (F x) , ⟨⟩) (fst (m (pt X) p)) u) ,
                ((prop-over-connected {{cX}} (λ x → has-level ⟨ S (S n) ⟩ (F x) , ⟨⟩) (fst (snd (m (pt X) p))) u) ,
                (λ x → ap (λ tr → Ω^'S-abgroup n ⊙[ F u , x ] {{tr}}) (prop-has-all-paths _ _) ∙' snd (snd (m u x))))))
              , p)
            (λ ((t , m) , p) →
