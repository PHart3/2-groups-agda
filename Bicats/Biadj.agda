...
module _ {i₁ i₂ j₁ j₂} {B₀ : Type i₁} {C₀ : Type i₂} {{ξB : BicatStr j₁ B₀}} {{ξC : BicatStr j₂ C₀}}
  ...
  record Biadj-data (ε : Pstrans (L ∘BC R) idfBC) (η : Pstrans idfBC (R ∘BC L)) :
    ...
    field
      -- the two triangulators (which are invertible modifications)
      ζ₁ : 
      ...
      -- the two swallowtail identities (which make the biadjunction fully coherent)
      ζ-zz₁ : (x : C₀) →
        ...
        ap (λ m → ⟦ ξC ⟧ η₀ ε x ◻ ⟦ ξC ⟧ η₀ ε (map-pf L (map-pf R x)) ◻ m) (lamb (F₁ (str-pf L) (η₀ η (map-pf R x)))) ∙
      ...
      ζ-zz₂ :  (x : B₀) →
        ...
        ap (λ m → ⟦ ξB ⟧ m ◻ η₀ η x)
          (ap (F₁ (str-pf R)) (ap (λ m → ⟦ ξC ⟧ η₀ ε (map-pf L x) ◻ m) (lamb (F₁ (str-pf L) (η₀ η x))))) ∙
