...
module _ {i₁ i₂ j₁ j₂} {B₀ : Type i₁} {C₀ : Type i₂} {{ξB : BicatStr j₁ B₀}} {{ξC : BicatStr j₂ C₀}}
  ...
  record InvMod (T₁ T₂ : Pstrans R S) : Type (lmax (lmax i₁ j₁) (lmax i₂ j₂)) where
    constructor pst-≃
    field
      η₀-∼ : (a : B₀) → η₀ T₁ a == η₀ T₂ a
      η₁-∼ : {a b : B₀} (f : hom a b) →
  ...
  InvMod-id : (T : Pstrans R S) → InvMod T T
  η₀-∼ (InvMod-id T) _ = idp
  η₁-∼ (InvMod-id T) f = idp
{-
  module _ {T₁ : Pstrans R S} where
    ...
    abstract
      InvMod-contr : is-contr (Σ (Pstrans R S) (λ T₂ → InvMod T₁ T₂))
      InvMod-contr = equiv-preserves-level lemma {{InvMod-contr-aux}}
        where
          lemma : tot-sp ≃ Σ (Pstrans R S) (λ T₂ → InvMod T₁ T₂)
          lemma =
            equiv
              (λ (η₀2 , η₁2) →
                pstrans (fst ∘ η₀2) (λ f → fst (η₁2 (_ , f))) ,
                pst-≃ (snd ∘ η₀2) (λ f → snd (η₁2 (_ , f))))
              (λ (pstrans η₀2 η₁2 , pst-≃ η₀-∼ η₁-∼) →
                (λ a → η₀2 a , η₀-∼ a) , (λ (_ , f) → (η₁2 f) , (η₁-∼ f)))
              (λ _ → idp)
              λ _ → idp

    InvMod-ind : ∀ {k} (P : (T₂ : Pstrans R S) → (InvMod T₁ T₂ → Type k))
      → P T₁ (InvMod-id T₁) → {T₂ : Pstrans R S} (e : InvMod T₁ T₂) → P T₂ e
    InvMod-ind P = ID-ind-map P InvMod-contr

    InvMod-ind-β : ∀ {k} (P : (T₂ : Pstrans R S) → (InvMod T₁ T₂ → Type k))
