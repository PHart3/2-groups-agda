...
module _ {{ξB : BicatStr j B₀}} where
  ...
  eps (snd (wkeqv-bc-promote (f , we))) =
    ...
    ! (lamb (f ◻ inv we))
  coher-map (snd (wkeqv-bc-promote (f , we))) = =ₛ-out $
    ρ f ◃∙
    ap (λ m → f ◻ m) (eta we) ◃∙
    α f (inv we) f ◃∎
    ...
    α (id₁ _) (f ◻ inv we) f ◃∙
    ap (λ m → m ◻ f) (! (lamb (f ◻ inv we))) ◃∎
      =ₛ⟨ 7 & 1 & apCommSq2◃-rev (λ k → ! (ap (λ m → f ◻ m) (α (inv we) f k)) ∙ α f (inv we) (⟦ ξB ⟧ f ◻ k)) (eta we) ⟩
    {!!} {-
    lamb f ◃∙
    ap (λ m → m ◻ f) (eps we) ◃∙
    ap (λ m → m ◻ f) (ap (λ m → f ◻ m) (lamb (inv we))) ◃∙
    ...
    α (id₁ _) (f ◻ inv we) f ◃∙
    ap (λ m → m ◻ f) (! (lamb (f ◻ inv we))) ◃∎
      =ₛ⟨ 8 & 1 & =ₛ-in (=ₛ-out (pent-bc◃-rot6 (inv we ◻ f) f (inv we) f)) ⟩
    lamb f ◃∙
    ap (λ m → m ◻ f) (eps we) ◃∙
    ap (λ m → m ◻ f) (ap (λ m → f ◻ m) (lamb (inv we))) ◃∙
    ! (α f (id₁ _ ◻ inv we) f) ◃∙
    ...
    α (id₁ _) (f ◻ inv we) f ◃∙
    ap (λ m → m ◻ f) (! (lamb (f ◻ inv we))) ◃∎
      =ₛ⟨ 10 & 1 & =ₛ-in (! (!-inv-r (α (⟦ ξB ⟧ ⟦ ξB ⟧ f ◻ inv we ◻ f) (inv we) f))) ⟩
    lamb f ◃∙
    ap (λ m → m ◻ f) (eps we) ◃∙
    ap (λ m → m ◻ f) (ap (λ m → f ◻ m) (lamb (inv we))) ◃∙
    ! (α f (id₁ _ ◻ inv we) f) ◃∙
    ...
    α (id₁ _) (f ◻ inv we) f ◃∙
    ap (λ m → m ◻ f) (! (lamb (f ◻ inv we))) ◃∎
      =ₛ⟨ 11 & 2 & !-∘-ap-∙◃ (λ m → ⟦ ξB ⟧ ⟦ ξB ⟧ f ◻ inv we ◻ m) (λ m → f ◻ m) (ρ f) (eta we) ⟩
    lamb f ◃∙
    ap (λ m → m ◻ f) (eps we) ◃∙
    ap (λ m → m ◻ f) (ap (λ m → f ◻ m) (lamb (inv we))) ◃∙
    ! (α f (id₁ _ ◻ inv we) f) ◃∙
    ...
    ! (α (f ◻ inv we) (f ◻ inv we) f) ◃∙
    ! (ap (λ m → ⟦ ξB ⟧ ⟦ ξB ⟧ f ◻ inv we ◻ m) (α f (inv we) f)) ◃∙
    ! (ap (λ m → ⟦ ξB ⟧ ⟦ ξB ⟧ f ◻ inv we ◻ m) (ρ f ∙ ap (λ m → f ◻ m) (eta we))) ◃∙
    ! (ap (λ m → m ◻ f) (eps we)) ◃∙
    ap (λ m → id₁ _ ◻ m) (ρ f ∙ ap (λ m → f ◻ m) (eta we) ∙ α f (inv we) f) ◃∙
    α (id₁ _) (f ◻ inv we) f ◃∙
    ap (λ m → m ◻ f) (! (lamb (f ◻ inv we))) ◃∎
    ...
    ap (λ m → m ◻ f) (! (lamb (f ◻ inv we))) ◃∎
      =ₛ⟨ 5 & 1 & ap-! (ap-∘ (λ m → f ◻ m) (λ m → id₁ _ ◻ m) (eta we)) ⟩
    lamb f ◃∙
    ap (λ m → m ◻ f) (eps we) ◃∙
    ap (λ m → m ◻ f) (ap (λ m → f ◻ m) (lamb (inv we))) ◃∙
    ! (α f (id₁ _ ◻ inv we) f) ◃∙
    ...
    ap (λ m → m ◻ f) (! (lamb (f ◻ inv we))) ◃∎
      =ₛ⟨ 3 & 3 &
        !ₛ (apCommSq◃ (λ m → ap (λ k → f ◻ k) (ρ m ∙ ap (λ k → m ◻ k) (eta we) ∙ α m (inv we) f) ∙ α f (m ◻ inv we) f) (eta we)) ⟩
    lamb f ◃∙
    ap (λ m → m ◻ f) (eps we) ◃∙
    ap (λ m → m ◻ f) (ap (λ m → f ◻ m) (lamb (inv we))) ◃∙
    ...
    ! (ap (λ m → ⟦ ξB ⟧ ⟦ ξB ⟧ m ◻ ⟦ ξB ⟧ f ◻ inv we ◻ f) (eps we)) ◃∙
    ap (λ m → m ◻ f) (! (lamb (f ◻ inv we))) ◃∎
      =ₛ₁⟨ 6 & 1 & !-ap (λ m → ⟦ ξB ⟧ ⟦ ξB ⟧ m ◻ ⟦ ξB ⟧ f ◻ inv we ◻ f) (eps we) ⟩ 
    lamb f ◃∙
    ap (λ m → m ◻ f) (eps we) ◃∙
    ap (λ m → m ◻ f) (ap (λ m → f ◻ m) (lamb (inv we))) ◃∙
    ap (λ m → m ◻ f) (ap (λ m → ⟦ ξB ⟧ f ◻ ⟦ ξB ⟧ m ◻ inv we) (eta we)) ◃∙
