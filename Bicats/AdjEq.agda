...
module _ {{ξB : BicatStr j B₀}} where
  ...
  module _ {a b : B₀} ((f , we) : WkEquiv-bc ξB a b) where
    ...
    wkeqv-refine0 = 
      ...
      α (id₁ _) (f ◻ inv we) f ◃∙
      ap (λ m → m ◻ f) (! (lamb (f ◻ inv we))) ◃∎
        =ₛ⟨ 1 & 1 & =ₛ-in (! (!-inv-r (ap (λ m → m ◻ f) (eps we)))) ⟩
      lamb f ◃∙
      ap (λ m → m ◻ f) (eps we) ◃∙
      idp ◃∙
      ! (ap (λ m → m ◻ f) (eps we)) ◃∙
      ...
      α (id₁ _) (f ◻ inv we) f ◃∙
      ap (λ m → m ◻ f) (! (lamb (f ◻ inv we))) ◃∎
        =ₛ⟨ 4 & 1 & =ₛ-in (ap (ap (λ m → f ◻ m)) (! (=ₛ-out (trig-ρ-bc-rot-pre (inv we) f)))) ⟩
      lamb f ◃∙
      ap (λ m → m ◻ f) (eps we) ◃∙
      ! (α f (inv we) f) ◃∙
      idp ◃∙
      ...
      α (id₁ _) (f ◻ inv we) f ◃∙
      ap (λ m → m ◻ f) (! (lamb (f ◻ inv we))) ◃∎
        =ₛ⟨ 3 & 1 & =ₛ-in (ap (ap (λ m → f ◻ m)) (! (=ₛ-out (trig-lamb-bc-rot (inv we) f)))) ⟩
      lamb f ◃∙
      ap (λ m → m ◻ f) (eps we) ◃∙
      ! (α f (inv we) f) ◃∙
      ap (λ m → f ◻ m)
