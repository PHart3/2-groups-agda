...
Πᴳ : ∀ {i j} (J : Type i) (F : J → Group j) → Group (lmax i j)
Πᴳ J F = group (Π J (El ∘ F)) {{Π-level (λ i → El-level (F i))}} -- TODO: how to get instance search to work?
               (Π-group-struct (group-struct ∘ F))
  where open Group

{- functorial behavior of [Πᴳ] -}
Πᴳ-emap-r : ∀ {i j k} (J : Type i) {F : J → Group j} {G : J → Group k}
  → (∀ i → F i ≃ᴳ G i) → Πᴳ J F ≃ᴳ Πᴳ J G
Πᴳ-emap-r J {F} {G} iso = ≃-to-≃ᴳ (Π-emap-r (GroupIso.f-equiv ∘ iso))
...
Πᴳ-is-abelian : ∀ {i j} {J : Type i} {F : J → Group j}
  → (∀ i → is-abelian (F i)) → is-abelian (Πᴳ J F)
...
Πᴳ-fanout : ∀ {i j k} {J : Type i} {G : Group j} {F : J → Group k}
  → ((i : J) → G →ᴳ F i) → (G →ᴳ Πᴳ J F)
...
Πᴳ-proj : ∀ {i j} {J : Type i} {F : J → Group j} (i : J)
  → (Πᴳ J F →ᴳ F i)
...
module _ where
  Πᴳ-is-trivial : ∀ {i j} (J : Type i) (F : J → Group j)
    → (∀ (i : J) → is-trivialᴳ (F i)) → is-trivialᴳ (Πᴳ J F)
  Πᴳ-is-trivial J F F-is-trivial = λ f → λ= λ i → F-is-trivial i (f i)
