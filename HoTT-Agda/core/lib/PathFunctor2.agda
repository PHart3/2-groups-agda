...
module _ {ℓ₁ ℓ₂} {A : Type ℓ₁} {B : Type ℓ₂} {x y : A} (p : x == y) where
    ...
    hnat-∙'-∙3 : {f₁ f₂ f₃ f₄ : A → B}
      ...
      ap ((λ q → H₁ x ∙ q ∙' ! (H₁ y)) ∘ (λ q → H₂ x ∙ q ∙' ! (H₂ y))) (hmtpy-nat-∙' H₃ p) ◃∙
    ...
    hnat-∙'-∙3 {f₄ = f₄} H₁ H₂ H₃ =
      ...
      _
        =ₛ₁⟨ 2 & 1 & ∘-ap (λ q → H₁ x ∙ q ∙' ! (H₁ y)) (λ q → H₂ x ∙ q ∙' ! (H₂ y)) (hmtpy-nat-∙' H₃ p) ⟩
      hmtpy-nat-∙' H₁ p ◃∙
      ap (λ q → H₁ x ∙ q ∙' ! (H₁ y)) (hmtpy-nat-∙' H₂ p) ◃∙
      ap ((λ q → H₁ x ∙ q ∙' ! (H₁ y)) ∘ (λ q → H₂ x ∙ q ∙' ! (H₂ y))) (hmtpy-nat-∙' H₃ p) ◃∙
      ap (λ q → H₁ x ∙ q ∙' ! (H₁ y)) (hnat-∙'̇-∙-aux (H₂ x) (H₃ x) (ap (λ z → f₄ z) p) (H₃ y) (H₂ y)) ◃∙
    ...
    hnat-∙'-∙4 {f₅ = f₅} H₁ H₂ H₃ H₄ =
      ...
      _
        =ₛ₁⟨ 3 & 1 & 
          ∘-ap (λ q → H₁ x ∙ q ∙' ! (H₁ y))
            ((λ q → H₂ x ∙ q ∙' ! (H₂ y)) ∘ (λ q → H₃ x ∙ q ∙' ! (H₃ y))) (hmtpy-nat-∙' H₄ p) ⟩
      hmtpy-nat-∙' H₁ p ◃∙
      ap (λ q → H₁ x ∙ q ∙' ! (H₁ y)) (hmtpy-nat-∙' H₂ p) ◃∙
      ap ((λ q → H₁ x ∙ q ∙' ! (H₁ y)) ∘ (λ q → H₂ x ∙ q ∙' ! (H₂ y))) (hmtpy-nat-∙' H₃ p) ◃∙
      ap ((λ q → H₁ x ∙ q ∙' ! (H₁ y)) ∘ (λ q → H₂ x ∙ q ∙' ! (H₂ y)) ∘ (λ q → H₃ x ∙ q ∙' ! (H₃ y)))
    ...
    hnat-∙'-∙5 : {f₁ f₂ f₃ f₄ f₅ f₆ : A → B}
      ...
      ap (λ q → H₁ x ∙ q ∙' ! (H₁ y)) (hmtpy-nat-∙' H₂ p) ◃∙
      ap ((λ q → H₁ x ∙ q ∙' ! (H₁ y)) ∘ (λ q → H₂ x ∙ q ∙' ! (H₂ y))) (hmtpy-nat-∙' H₃ p) ◃∙
      ap ((λ q → H₁ x ∙ q ∙' ! (H₁ y)) ∘ (λ q → H₂ x ∙ q ∙' ! (H₂ y)) ∘ (λ q → H₃ x ∙ q ∙' ! (H₃ y))) (hmtpy-nat-∙' H₄ p) ◃∙
      ap
...
module _ {ℓ₁ ℓ₂ ℓ₃} {A : Type ℓ₁} {B : Type ℓ₂} {C : Type ℓ₃} (g : B → C)
  ...
  private
    μ₁ = ap ! (∘-∘-ap (λ q → ! (ap g (H₁ x ∙ H₂ x)) ∙ q ∙' ! (! (ap g (H₁ y ∙ H₂ y)))) (ap g) (λ q → H₁ x ∙ q ∙' ! (H₁ y)) (hmtpy-nat-∙' H₂ p))
    μ₂ = ap ! (∘-ap (λ q → ! (ap g (H₁ x ∙ H₂ x)) ∙ q ∙' ! (! (ap g (H₁ y ∙ H₂ y)))) (ap g) (hmtpy-nat-∙' H₁ p))

  hnat-∙'-!ap∙ :
    hmtpy-nat-∙' (λ x → ! (ap g (H₁ x ∙ H₂ x))) p ◃∎
      =ₛ
    ...
    ! (ap ((λ q → ! (ap g (H₁ x ∙ H₂ x)) ∙ q ∙' ! (! (ap g (H₁ y ∙ H₂ y)))) ∘ ap g ∘ (λ q → H₁ x ∙ q ∙' ! (H₁ y))) (hmtpy-nat-∙' H₂ p)) ◃∙
    ! (ap (λ q → ! (ap g (H₁ x ∙ H₂ x)) ∙ ap g q ∙' ! (! (ap g (H₁ y ∙ H₂ y)))) (hmtpy-nat-∙' H₁ p)) ◃∙
  ...
  hnat-∙'-!ap∙ =
    ...
    ! (ap (λ q → ! (ap g (H₁ x ∙ H₂ x)) ∙ q ∙' ! (! (ap g (H₁ y ∙ H₂ y)))) (ap (ap g) (hnat-∙'̇-∙-aux (H₁ x) (H₂ x) (ap f₃ p) (H₂ y) (H₁ y)))) ◃∙
    ! (ap ((λ q → ! (ap g (H₁ x ∙ H₂ x)) ∙ q ∙' ! (! (ap g (H₁ y ∙ H₂ y)))) ∘ ap g ∘ (λ q → H₁ x ∙ q ∙' ! (H₁ y))) (hmtpy-nat-∙' H₂ p)) ◃∙
    ! (ap (λ q → ! (ap g (H₁ x ∙ H₂ x)) ∙ q ∙' ! (! (ap g (H₁ y ∙ H₂ y)))) (ap (ap g) (hmtpy-nat-∙' H₁ p))) ◃∙
    ! (ap (λ q → ! (ap g (H₁ x ∙ H₂ x)) ∙ q ∙' ! (! (ap g (H₁ y ∙ H₂ y)))) (ap-∘ g f₁ p)) ◃∎
      =ₛ₁⟨ 5 & 1 & μ₂ ⟩
    hnat-∙'̇-!-aux (ap (g ∘ f₃) p) (ap g (H₁ x ∙ H₂ x)) (ap g (H₁ y ∙ H₂ y)) ◃∙
    ! (ap (λ q → ! (ap g (H₁ x ∙ H₂ x)) ∙ q ∙' ! (! (ap g (H₁ y ∙ H₂ y)))) (ap (λ q → ap g (H₁ x ∙ H₂ x) ∙ q ∙' ! (ap g (H₁ y ∙ H₂ y))) (∘-ap g f₃ p))) ◃∙
